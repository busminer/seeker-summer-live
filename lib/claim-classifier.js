const { lookup, learn } = require('./camp-stats');
...
async function classify(signature, amount) {
  if (!signature) return { staked: null, route: 'unknown' };
  if (cache.has(signature)) return cache.get(signature);
  const known = lookup(signature);
  if (known && (known.r === 'staking' || known.r === 'summer')) {
    const result = { staked: known.r === 'staking', route: known.r };
  ...
  cache.set(signature, result);
  learn(signature, result.route, amount);
...
async function enrichClaims(claims, limit = 32, concurrency = 5) {
  ...
  async function worker() {
    while (cursor < Math.min(limit, output.length)) {
      const index = cursor++;
      Object.assign(output[index], await classify(output[index].signature, output[index].amount));
    }
  }
